  ...
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        <Badge
          variant="secondary"
          className="flex items-center gap-1 px-2 py-1 rounded-full hover:bg-secondary/80 dark:bg-gray-800/80 transition-colors duration-200"
        >
          {getIcon(tech)}
...
const ExperienceCard: React.FC<{ item: ExperienceItem }> = ({ item }) => (
  <Card className="group h-full bg-card hover:bg-card/90 dark:bg-gray-800/80 transition-colors duration-300">
    <CardHeader className="pb-2 dark:bg-gray-800/80">
      <CardTitle className="text-lg font-semibold flex items-center gap-2 text-card-foreground dark:bg-gray-800/80">
        <BadgeCheck className="text-primary" />
        {item.name}
      </CardTitle>
    </CardHeader>
    <CardContent className="pt-4 relative">
      <p className="text-sm text-black-foreground mb-3">{item.subtitle}</p>
      {item.value !== undefined && (
        <div className="mb-3">
          <Progress value={item.value} className="h-2" />
          <p className="text-xs text-right mt-1 text-black-foreground">
            {item.value}%
          </p>
        </div>
      )}
      ...
      {item.experience && (
        ...
        >
          <p className="text-sm text-black-foreground">{item.experience}</p>
        </motion.div>
      )}
    </CardContent>
  </Card>
    ...
    >
      <div className="container mx-auto px-4">
        ...
        <Tabs defaultValue="services" className="w-full ">
          <TabsList className="flex flex-auto justify-center gap-2 mb-8 md:mb-8 dark:bg-gray-800/80">
            <TabsTrigger
              value="services"
